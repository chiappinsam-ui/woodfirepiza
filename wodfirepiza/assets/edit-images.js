(() => {
  ...
  async function applyManifestToImages() {
    ...
    for (const img of imgs) {
      ...
      if (manifest[slot]) {
        // FIX: Check for public_url. If it exists, go DIRECT to Supabase.
        const directLink = manifest[slot].public_url;
        const localLink = `/media/${encodeURIComponent(slot)}?v=${manifest[slot].updated || Date.now()}`;
        
        forceImgSrc(img, directLink || localLink);
      }
    }
  }
  ...
  async function applyManifestToBackgrounds() {
    ...
    document.querySelectorAll("[data-bg-slot]").forEach((el) => {
      ...
      // FIX: Use direct Supabase link here too
      const directLink = info.public_url;
      const localLink = `/media/${encodeURIComponent(slot)}?v=${info.updated || Date.now()}`;
      
      el.style.backgroundImage = `url("${directLink || localLink}")`;
    });
  }
  ...
  async function doUpload(slot, file) {
    ...
    // IMPORTANT: Return the JSON so we can get the public_url
  ...
  picker.addEventListener("change", async () => {
    ...
    try {
      // 1. Upload to server (which uploads to Supabase)
      const data = await doUpload(state.pickerSlot, file);
      
      // 2. Use the fast Supabase URL if available, otherwise fallback to local
      const newUrl = data.public_url || `/media/${encodeURIComponent(state.pickerSlot)}?v=${Date.now()}`;

      // 3. Update the image immediately
      if (state.pickerImg) {
